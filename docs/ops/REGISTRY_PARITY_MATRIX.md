...
| MethodologyVersion | `/catalog/methodology-versions/` | — | `/catalog/methodology-versions/new/` | `/catalog/methodology-versions/<uuid>/edit/` | Detail page is missing (versions are shown under the parent methodology detail view). | List filtered via `filter_methodologies_for_user` + `audit_queryset_access`; create requires contributor; edit requires `can_edit_methodology` on parent and `audit_sensitive_access` on version. |

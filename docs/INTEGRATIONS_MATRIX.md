...
| BIRDIE external pipeline integration | BIRDIE represented as an operational programme scaffold (`NBMS-BIRDIE-INTEGRATION`) seeded by `seed_programme_ops_v1`, with connector-oriented pipeline definition and run lineage model | Full `integrations/birdie/` API client module with bronze/silver/gold ingest and indicator marts |
